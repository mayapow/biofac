...
jp5_annotation <- function(eem_plot, eem_indx, sample_id = paste("Sample:", eem_indx$UniqueID)){
...
jp5_severity_plot <- function(jp5_value,
  ...
  ggplot(df, aes(y = jp5, x = sample, color = severity)) + 
...
plot_ex = function(excitation_nm, eem, sample_name){
  ...
  # first column is emission, remaining columns are excitation wavelengths
  exs = as.numeric(names(eem)[-1])
  ems = eem[[1]]
  ...
  ex_dat = data.frame(ems = ems, value = eem[,closest_ex+1])
