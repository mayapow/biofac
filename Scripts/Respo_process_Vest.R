###### Respo Code for PI Light and Dark Runs ####### 
...
#Plot curves
pcom_resp <- RespoR_Normalized %>% filter(species == "PCOM")
mcap_resp <- RespoR_Normalized %>% filter(species == "MCAP")

pcom.mean <- aggregate(umol.cm2.hr ~ Light_value, data = pcom_resp, FUN=mean)
pcom.se <- aggregate(umol.cm2.hr ~ Light_value, data = pcom_resp, FUN=std.error)

mcap.mean <- aggregate(umol.cm2.hr ~ Light_value, data = mcap_resp, FUN=mean)
mcap.se <- aggregate(umol.cm2.hr ~ Light_value, data = mcap_resp, FUN=std.error)
...
#PCOM Data
PAR <- as.numeric(pcom_resp$Light_value)
Pc <- as.numeric(pcom_resp$umol.cm2.hr)
plot(PAR,Pc,xlab="", ylab="", xlim=c(0,max(PAR)), ylim=c(-0.5, 1.2), cex.lab=0.8,cex.axis=0.8,cex=1, main="Porites compressa", adj = 0.05) #set plot info
...
Pcom.PI.Output <- rbind(Pmax.gross, Pmax.net, -Rd, AQY,Ik,Ic)
row.names(Pcom.PI.Output) <- c("Pg.max","Pn.max","Rdark","alpha", "Ik", "Ic")
...
#PI curve for MCAP
PAR <- as.numeric(mcap_resp$Light_value)
Pc <- as.numeric(mcap_resp$umol.cm2.hr)
plot(PAR,Pc,xlab="", ylab="", xlim=c(0,max(PAR)), ylim=c(-0.5, 2), cex.lab=0.8,cex.axis=0.8,cex=1, main="Montipora capitata", adj = 0.05) #set plot info
mtext(expression("Irradiance ("*mu*"mol photons "*m^-2*s^-1*")"),side=1,line=3.3,cex=1) #add labels
mtext(expression(Rate*" ("*mu*"mol "*O[2]*" "*cm^-2*h^-1*")"),side=2,line=2,cex=1) #add labels
...
#draw the curve using the model fit
mcap.curve.fitting <- curve((1/(2*summary(curve.nlslrc)$coef[4,1]))*(summary(curve.nlslrc)$coef[2,1]*x+summary(curve.nlslrc)$coef[1,1]-sqrt((summary(curve.nlslrc)$coef[2,1]*x+summary(curve.nlslrc)$coef[1,1])^2-4*summary(curve.nlslrc)$coef[2,1]*summary(curve.nlslrc)$coef[4,1]*summary(curve.nlslrc)$coef[1,1]*x))-summary(curve.nlslrc)$coef[3,1],lwd=2,col="blue",add=T)
#dev.off()
...
#output parameters into a table
mcap.PI.Output <- rbind(Pmax.gross, Pmax.net, -Rd, AQY,Ik,Ic)
row.names(mcap.PI.Output) <- c("Pg.max","Pn.max","Rdark","alpha", "Ik", "Ic")
