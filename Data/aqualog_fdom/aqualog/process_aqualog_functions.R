...
organize_aqualog <- function(data_directory,
  ...
  blank_files <- sort(grep("Sample0001BEM", dat_files, value = TRUE))
  SEM_files <- sort(grep("SEM", dat_files, value = TRUE))
  ABS_files <- sort(grep("ABS", dat_files, value = TRUE))
  ...
  if(N_blanks  < 1)  stop("No blank BEM files found in the directory!")
  ...
  sample_key <- sample_key[!(is.na(sample_key$UniqueID) | trimws(sample_key$UniqueID) == ""), ]
...
correct_EEMs <- function(data_directory, 
  ...
  has_abs <- "ABS_file" %in% colnames(sample_sheet) &&
    all(!is.na(sample_sheet$ABS_file)) &&
    all(sample_sheet$ABS_file != "")
...
fdom_indices <- function(Sample_EEMs,
  ...
  # Calculate complex indices for each sample
    ...
    # M to C ratio
    indices_out[["M_to_C"]] <-  
                                indices_out[["simple"]][ , "CobleM"] / indices_out[["simple"]][ , "CobleC"]
...
read_eems = function(eem_dir, pattern = "clean.csv"){
  eem_files = sort(list.files(eem_dir))
...
read_processed_data = function(run_out_dir){
  ...
  if(!(length(indices_file) == 1)) stop(paste("ERROR check for single indices file", run_out_dir))
  
  run_sheet   = read.csv(sample_path, header = T, stringsAsFactors = F)
  run_indices = read.csv(indices_path, header = T, stringsAsFactors = F)
  
  if(!all(run_indices$UniqueID %in% run_sheet$UniqueID)){
    warning(
      paste("WARNING Samples in indices don't match run sample sheet", run_out_dir),
      " Samples will be dropped: ",
      paste(run_indices$UniqueID[!(run_indices$UniqueID %in% run_sheet$UniqueID)],
            collapse = ", ")
    )
    
    run_indices <- run_indices[run_indices$UniqueID %in% run_sheet$UniqueID, ]
    run_sheet <- run_sheet[run_sheet$UniqueID %in% run_indices$UniqueID, ]
  }
...
compile_runs <- function(run_dirs,
  ...
  # sample metadata
  run_samples <- lapply(processed_runs, function(x) x$samples)
  sample_sheet <- do.call("rbind", run_samples)
  
  compiled_indices_small <- compiled_indices %>%
    dplyr::select(-run_name, -sample)
  
  all_samples <- merge(sample_sheet, compiled_indices_small, by = "UniqueID", all.y = TRUE)
  
  if(nrow(compiled_indices) != nrow(all_samples)) {
    stop("ERROR sample indices merge error")
  }
